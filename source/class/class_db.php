...
class db_mysql {
	...
	function connect() {
		global $_config;
		$this->curlink = $this->_dbconnect ( $_config ['db'] ['server'] . ":" . $_config ['db'] ['port'], $_config ['db'] ['username'], $_config ['db'] ['password'], $_config ['db'] ['charset'], $_config ['db'] ['name'], $_config ['db'] ['pconnect'] );
	}
