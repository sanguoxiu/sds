...
<div class="bind">
	...
	<div id="bind_r" class="bind_f">
       <span>欢迎您，<a target="_blank" href="<?php echo $_SESSION['sina_url'];?>" title="进入我的微博"><?php echo $_SESSION['wname'];?></a> 请输入用户名、密码、邮箱后绑定</span>
		<form method="post" action="member.php?action=sina_bind">
            <input type="text" name="username" placeholder="用户名" value="<?php echo $_SESSION['wname'];?>" required  />
	...
	<div id="bind_l" class="bind_f hide">
        <span>欢迎您，<a target="_blank" href="<?php echo $_SESSION['sina_url'];?>" title="进入我的微博"><?php echo $_SESSION['wname'];?></a> 请输入用户名、密码。登陆后绑定</span>
		<form method="post" action="member.php?action=sina_bind">
